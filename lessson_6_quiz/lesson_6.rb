...
# ФАЙЛЫ YAML , INJECT , ord / chr . работа с датой
...
# YAML - формат хранения данных, которые далее используются в коде. yet anorther markup language 
# ямл работает в формате ключ значение 

# создание квиза )
# ввод имя с клавиатуры
# puts 'Введите ваше имя'
# name = gets.strip

# взять вопросы и ответы
...
all_questions.shuffle.each do |question_data|
  ...
  # 3 переменные в  do |result, (answer, i)|
  ...
  loop do # это цикл, который просто запрашивает ввод с клавиатуры
    ...
    if user_answer_char.between?('A', 'D') # что ответ вообще в диапазоне от А до D через метод between
      break
    else
